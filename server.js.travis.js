...
app.delete('/todos/:id', function(req, res){
    ...
    db.todo.destroy({
        ...
        if(rowsDeleted > 0){
            res.json("Row with ID: " + todoId + " deleted successfully");
        } else{
            res.status(404).json("No rows found to delete");
        }
    }, function(err){
...
app.put('/todos/:id', function(req, res){
    ...
    db.todo.findById(todoId).then(function(todo){  //findById went well
        if(todo){
            return todo.update(attributes); //update and return promise further up chain
        } else{
            response.status(404).send();
        }
    }, function(){
        res.status(500); //not found (findbyid failed)
    }).then(function(todo){  //handle promise returned by update; went well
        res.json(todo.toJSON()); // 
    }, function(e){ // error with update
        res.status(400).json(e);
    });
